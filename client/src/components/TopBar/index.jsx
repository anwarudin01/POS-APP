    ...
    <Navbar bg="primary" variant="dark" expand="lg" fixed="top">
      <Container>
        <IndexLinkContainer to="/" exact>
          <Navbar.Brand>Milagro POS App</Navbar.Brand>
        </IndexLinkContainer>
          ...
          <Nav className="me-auto my-2 my-lg-0 mr-3">
            <NavDropdown title={products.category || 'Kategori'} id="navbarScrollingDropdown">
              {categories.map((category, i) => (
                <NavDropdown.Item href="#" key={i} active={category.name === products.category} onClick={() => dispatch(setCategory(category.name))}>
                  {category.name}
                </NavDropdown.Item>
              ))}
            </NavDropdown>
